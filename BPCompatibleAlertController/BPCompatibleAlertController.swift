...
public class BPCompatibleAlertController : NSObject, UIAlertViewDelegate {
    ...
    */
    ...
    public func presentPopoverFrom(viewController: UIViewController!, view: UIView, animated: Bool, completion: (() -> Void)?) {
        presentFrom(viewController, animated: animated, completion: completion)
        if #available(iOS 8.0, *) {
            let alertController = self.alerter as! BPAlertController
